...
export default function TaskCenterPage() {
  ...
  const [paymentTab, setPaymentTab] = useState<string>("patients")
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Main Content Area */}
      <div className="flex-1" style={{ backgroundColor: "rgb(237, 245, 250)" }}>
        {activeTab === "Payments" ? (
          <div className="w-full p-8">
            <Tabs value={paymentTab} onValueChange={setPaymentTab} className="w-full">
              <TabsList className="inline-flex gap-1 bg-transparent mb-6">
                <TabsTrigger
                  value="patients"
                  className="w-[200px] py-2 text-sm bg-gray-200 text-gray-700 data-[state=active]:bg-[#FF8F5C] data-[state=active]:text-white rounded-t-lg border-0"
                >
                  From Patient
                </TabsTrigger>
                <TabsTrigger
              ...
              <TabsContent value="patients" className="mt-0">
                <PaymentList source="Patients" />
              </TabsContent>

              <TabsContent value="insurance" className="mt-0">
                <PaymentList source="Insurance" />
              </TabsContent>
            </Tabs>
          </div>
        ) : (
